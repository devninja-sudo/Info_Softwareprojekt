...
class Springer(FigurBuilder):
    ...
    def getMaybePossibleTurns(self, originFieldLabel:str)->list[dict]:
        ...
        for i in [-1, 1]:
            for j in [-1, 1]:
                possibleZuege = self.getNewZugListWithAddingRelative(originFieldLabel, possibleZuege, (self.__SchrittLaenge*i, self.__SchrittBreite*j), self.__mustKill)
                possibleZuege = self.getNewZugListWithAddingRelative(originFieldLabel, possibleZuege, (self.__SchrittBreite*i, self.__SchrittLaenge*j), self.__mustKill)
